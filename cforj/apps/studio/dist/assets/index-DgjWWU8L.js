import{J as a}from"./jszip.min-CZKwHina.js";import"./index-GGhvJkoj.js";function d(e,t){const n=t==="1.2",r=n?"<schema>ADL SCORM</schema><schemaversion>1.2</schemaversion>":"<schema>ADL SCORM</schema><schemaversion>2004 4th Edition</schemaversion>";return`<?xml version="1.0" encoding="UTF-8"?>
<manifest identifier="app-${e.id}" version="1"
  xmlns="http://www.imsproject.org/xsd/imscp_rootv1p1p2"
  xmlns:adlcp="${n?"http://www.adlnet.org/xsd/adlcp_rootv1p2":"http://www.adlnet.org/xsd/adlcp_v1p3"}"
  ...
  <organizations default="org-${e.id}">
    <organization identifier="org-${e.id}">
  ...
  <resources>
    <resource identifier="res-main" type="webcontent" ${n?'adlcp:scormtype="sco"':'adlcp:scormType="sco"'} href="index.html">
      <file href="index.html"/>
      <file href="course.json"/>
    </resource>
  </resources>

</manifest>`}function o(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&apos;")}function x(e){return`<?xml version="1.0" encoding="UTF-8"?>
<tincan xmlns="http://projecttincan.com/tincan.xsd">
  <activities>
    <activity id="${`http://course-studio/${e.id}`}" type="http://adlnet.gov/expapi/activities/course">
      <name lang="${e.language??"en"}">${i(e.title)}</name>
      <description lang="${e.language??"en"}">${i(e.description??"")}</description>
      <launch lang="${e.language??"en"}">index.html</launch>
    </activity>
  </activities>
</tincan>`}function b(e){const t=`http://course-studio/${e.id}`,n=JSON.stringify(e);return`<!DOCTYPE html>
<html lang="${i(e.language??"en")}">
<head>
  <meta charset="UTF-8">
...
var ENDPOINT=null;
var AUTH=null;
try{var p=new URLSearchParams(window.location.search);var a=p.get('actor');if(a)ACTOR=JSON.parse(decodeURIComponent(a));ENDPOINT=p.get('endpoint');AUTH=p.get('auth');}catch(e){}
function sendStatement(verb,result){if(!ACTOR||!ENDPOINT)return;var stmt={actor:ACTOR,verb:verb,object:{id:ACTIVITY_ID,objectType:'Activity'}};if(result)stmt.result=result;fetch(ENDPOINT+'statements',{method:'POST',headers:{'Content-Type':'application/json','X-Experience-API-Version':'1.0.3','Authorization':AUTH?decodeURIComponent(AUTH):''},body:JSON.stringify(stmt)}).catch(function(){});}
sendStatement({id:'http://adlnet.gov/expapi/verbs/launched',display:{en:'launched'}});
var COURSE=${n};
...
function next(){if(state.screenIndex<COURSE.screens.length-1){state.screenIndex++;render();}else{state.completed=true;sendStatement({id:'http://adlnet.gov/expapi/verbs/completed',display:{en:'completed'}},{score:{raw:state.score},completion:true,success:true});render();}}
...
</html>`}function i(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}async function g(e,t={}){const{scormVersion:n="1.2"}=t,r=new a;return r.file("imsmanifest.xml",d(e,n)),r.file("course.json",JSON.stringify(e,null,2)),r.file("index.html",l(e,n)),r.generateAsync({type:"blob",mimeType:"application/zip"})}function l(e,t){const n=JSON.stringify(e),r=t==="1.2"?"var API = window.API || parent.API; if(API) API.LMSInitialize('');":"var API = window.API_1484_11 || parent.API_1484_11; if(API) API.Initialize('');",s=t==="1.2"?"if(API){API.LMSSetValue('cmi.core.lesson_status','passed');API.LMSSetValue('cmi.core.score.raw',String(state.score));API.LMSFinish('');}":"if(API){API.SetValue('cmi.completion_status','completed');API.SetValue('cmi.success_status','passed');API.SetValue('cmi.score.raw',String(state.score));API.Terminate('');}";return`<!DOCTYPE html>
<html lang="${c(e.language??"en")}">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
try{${r}}catch(scormErr){console.log('SCORM API not available (standalone mode)');}
var scormFinishCode=function(){try{${s}}catch(e){}};
...
</html>`}function c(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}async function v(e){const t=p(e);return new Blob([t],{type:"text/html;charset=utf-8"})}function p(e){const t=JSON.stringify(e);return`<!DOCTYPE html>
<html lang="${c(e.language??"en")}">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}export{v as exportHtml,g as exportScorm,d as generateImsmanifest,x as generateTinCanXml,b as generateXApiIndexHtml};
