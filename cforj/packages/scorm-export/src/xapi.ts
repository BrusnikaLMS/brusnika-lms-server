...
export function generateTinCanXml(app: CourseApp): string {
  const activityId = `http://course-studio/${app.id}`
  return `<?xml version="1.0" encoding="UTF-8"?>
<tincan xmlns="http://projecttincan.com/tincan.xsd">
  <activities>
    <activity id="${activityId}" type="http://adlnet.gov/expapi/activities/course">
      <name lang="${app.language ?? 'en'}">${escapeXml(app.title)}</name>
      <description lang="${app.language ?? 'en'}">${escapeXml(app.description ?? '')}</description>
      <launch lang="${app.language ?? 'en'}">index.html</launch>
    </activity>
  </activities>
</tincan>`
}

export function generateXApiIndexHtml(app: CourseApp): string {
  const activityId = `http://course-studio/${app.id}`
  const courseJson = JSON.stringify(app)

  return `<!DOCTYPE html>
<html lang="${escapeXml(app.language ?? 'en')}">
<head>
  <meta charset="UTF-8">
